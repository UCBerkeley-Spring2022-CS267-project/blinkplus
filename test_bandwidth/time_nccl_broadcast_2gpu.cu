...
int main(int argc, char* argv[])
{
    if ( argc != 6 )
    {
        printf("Usage ./time_nccl_broadcast GU1 GPU2 NUM_WARMUP NUM_ITER TOTAL_DATA_SIZE\n");
        exit(1);
    }

    setenv( "NCCL_PROTO", "Simple", 1);
    setenv( "NCCL_ALGO", "Ring", 1 );
    setenv( "NCCL_GRAPH_DUMP_FILE", "graph.xml", 1 );
    ...
    std::vector<int> user_group_devs = { atoi(argv[1]), atoi( argv[2]) };
