...
int main(int argc, char* argv[])
{
    if ( argc != 6 )
    {
        printf("Usage ./time_nccl_allreduce_user01_3group.out 0 1 NUM_WARMUP NUM_ITER TOTAL_DATA_SIZE\n");
        exit(1);
    }
    ...
    for ( int comm_i = 0; comm_i < helper_group_1_num_comm; ++comm_i )
    {
      ...
      {
        CUDACHECK(cudaMemset( helper_group_1_sendbuff[ comm_i ], 0, chunk_data_size * sizeof(uint8_t)));    
      }
    ...
    for ( int comm_i = 0; comm_i < helper_group_2_num_comm; ++comm_i )
    {
      ...
      {
        CUDACHECK(cudaMemset( helper_group_2_sendbuff[ comm_i ], 0, chunk_data_size * sizeof(uint8_t)));    
      }
    ...
    for (int iter = 0; iter < num_warmup; iter++) 
    {
      ...
      for ( int i = 0; i < user_group_num_comm; ++i )
      {
          NCCLCHECK(ncclAllReduce((const void*)user_group_sendbuff[ i ], \
      ...
      for ( int i = 0; i < helper_group_1_num_comm; ++i )
      {
          NCCLCHECK(ncclAllReduce((const void*)helper_group_1_sendbuff[ i ], \
      ...
      for ( int i = 0; i < helper_group_2_num_comm; ++i ) 
      {
          NCCLCHECK(ncclAllReduce((const void*)helper_group_2_sendbuff[ i ], \
    ...
    // End timing
    {
      ...
      double bw = total_data_size / 3 * sizeof(uint8_t) / 1.0E9 / deltaSec;
      printf("%s:: User Average of %d Iters, data: %d MB,  Elapsed Time: %7.5f (us), BandWidth: %7.5f (GB/s)\n", \
                  __func__, num_iters, int(atoi(argv[5]) / 3 * sizeof(uint8_t)), timeUsec,  bw);  
    }
