...
int main(int argc, char* argv[])
{
    ...
    std::vector<int> user_group_devs = { 0, 1 };
    ...
    std::vector<int> helper_group_1_devs = { 0,2,1 };
    ...
    std::vector<int> helper_group_2_devs = { 0,3,1 };
    ...
    int helper_group_3_num_comm = 4;
    std::vector<int> helper_group_3_devs = { 0,3,2,1 };
    std::vector<ncclComm_t> helper_group_3_comms( helper_group_3_num_comm );
    std::vector<cudaStream_t> helper_group_3_streams( helper_group_3_num_comm );
    ...
    printf("%s:: Helper 3 GPU %d, %d, %d, %d\n", __func__, helper_group_3_devs[0], helper_group_3_devs[1], helper_group_3_devs[2], helper_group_3_devs[3]);

    printf("%s:: Init stream data\n", __func__ );
    for ( int i = 0; i < user_group_num_comm; ++i )
    {
        CUDACHECK(cudaSetDevice( user_group_devs[ i ] ));
        CUDACHECK(cudaStreamCreateWithFlags( &(user_group_streams[i]), cudaStreamNonBlocking ));
    }

    for ( int i = 0; i < helper_group_1_num_comm; ++i )
    {
        CUDACHECK(cudaSetDevice( helper_group_1_devs[ i ] ));
        CUDACHECK(cudaStreamCreateWithFlags( &(helper_group_1_streams[i]), cudaStreamNonBlocking ));
    }

    for ( int i = 0; i < helper_group_2_num_comm; ++i )
    {
        CUDACHECK(cudaSetDevice( helper_group_2_devs[ i ] ));
        CUDACHECK(cudaStreamCreateWithFlags( &(helper_group_2_streams[i]), cudaStreamNonBlocking ));
    }

    for ( int i = 0; i < helper_group_3_num_comm; ++i )
    {
        CUDACHECK(cudaSetDevice( helper_group_3_devs[ i ] ));
        CUDACHECK(cudaStreamCreateWithFlags( &(helper_group_3_streams[i]), cudaStreamNonBlocking ));
    }
    ...
    uint8_t** helper_group_3_sendbuff = (uint8_t**)malloc(helper_group_3_num_comm * sizeof(uint8_t*));
    uint8_t** helper_group_3_recvbuff = (uint8_t**)malloc(helper_group_3_num_comm * sizeof(uint8_t*));
    ...
    for ( int comm_i = 0; comm_i < helper_group_3_num_comm; ++comm_i )
    {
      CUDACHECK(cudaSetDevice( helper_group_3_devs[ comm_i ] ));
      CUDACHECK(cudaMalloc( (helper_group_3_sendbuff + comm_i), chunk_data_size * sizeof(uint8_t) ));
      CUDACHECK(cudaMalloc( (helper_group_3_recvbuff + comm_i), chunk_data_size * sizeof(uint8_t)));
      CUDACHECK(cudaMemcpy( helper_group_3_sendbuff[ comm_i ], h_sendbuff.data(), chunk_data_size * sizeof(uint8_t), cudaMemcpyHostToDevice ));
      CUDACHECK(cudaMemset( helper_group_3_recvbuff[ comm_i ], 0, chunk_data_size * sizeof(uint8_t)));    
    }
    ...
    setenv( "NCCL_GRAPH_FILE", std::getenv("BLINKPLUS_GRAPH_FILE_CHAIN_0321") , 1 );
    NCCLCHECK(ncclCommInitAll( helper_group_3_comms.data(), helper_group_3_num_comm, helper_group_3_devs.data() ));
    ...
    for (int iter = 0; iter < num_warmup; iter++) 
    {
      ...
      for ( int i = 0; i < helper_group_3_num_comm; ++i ) 
      {
          NCCLCHECK(ncclBroadcast((const void*)helper_group_3_sendbuff[ i ], \
                                  (void*)helper_group_3_recvbuff[ i ], \
                                  chunk_data_size, ncclInt8, helper_group_3_devs[ 0 ], \
                                  helper_group_3_comms[i], \
                                  helper_group_3_streams[i]));
      }
      NCCLCHECK(ncclGroupEnd());
    ...
    {
      CUDACHECK(cudaSetDevice( helper_group_3_devs[i] ));
      CUDACHECK(cudaStreamSynchronize( helper_group_3_streams[i] ));
    }
    ...
    //   deltaSec = deltaSec / num_iters;
    //   double timeUsec = deltaSec*1.0E6;
    //   double bw = total_data_size / 4 * 3 * sizeof(uint8_t) / 1.0E9 / deltaSec;
    //   printf("%s:: Average of %d Iters, data: %d MB,  Elapsed Time: %7.5f (us), BandWidth: %7.5f (GB/s)\n", \
    //               __func__, num_iters, int(atoi(argv[5]) / 4 * 3 * sizeof(uint8_t)), timeUsec,  bw);  
    // }
    ...
    for ( int comm_i = 0; comm_i < helper_group_3_num_comm; ++comm_i )
    {
      CUDACHECK( cudaMemcpy( h_recvbuff.data(), helper_group_3_recvbuff[ comm_i ], chunk_data_size * sizeof( uint8_t ), cudaMemcpyDeviceToHost ));
      for ( int i = 0; i < h_recvbuff.size(); ++i )
      {
        if ( h_recvbuff[i] != h_sendbuff[i] )
        {
    ...
    {
      CUDACHECK(cudaSetDevice( helper_group_3_devs[ comm_i ] ));
      CUDACHECK(cudaFree( helper_group_3_sendbuff[ comm_i ] ));
      CUDACHECK(cudaFree( helper_group_3_recvbuff[ comm_i ] ));
    }
    ...
    for ( int i = 0; i < helper_group_3_num_comm; ++i ) 
    {
        ncclCommDestroy( helper_group_3_comms[i]);
    }
