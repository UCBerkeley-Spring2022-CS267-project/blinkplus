...
int main(int argc, char* argv[])
{
    if ( argc != 6 )
    {
        printf("Usage ./time_nccl_broadcast_user02_2group 0 2 NUM_WARMUP NUM_ITER TOTAL_DATA_SIZE\n");
        exit(1);
    }
    ...
    int chunk_data_size = total_data_size / 2;
    ...
    // End timing
    {
      ...
      double bw = total_data_size / 2 * sizeof(uint8_t) / 1.0E9 / deltaSec;
      printf("%s:: Average of %d Iters, data: %d MB,  Elapsed Time: %7.5f (us), BandWidth: %7.5f (GB/s)\n", \
                  __func__, num_iters, int(atoi(argv[5]) / 2 * sizeof(uint8_t)), timeUsec,  bw);  
    }
