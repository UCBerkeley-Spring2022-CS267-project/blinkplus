...
int main(int argc, char* argv[])
{
    if ( argc != 6 )
    {
        printf("Usage ./time_nccl_broadcast_user02_3group 0 2 NUM_WARMUP NUM_ITER TOTAL_DATA_SIZE\n");
        exit(1);
    }
    ...
    std::vector<int> user_group_devs = { 0,2 };
    ...
    std::vector<int> helper_group_1_devs = { 0,3,2 };
    ...
    std::vector<int> helper_group_2_devs = { 0,3,2 };
    ...
    int helper_group_3_num_comm = 3;
    std::vector<int> helper_group_3_devs = { 0,1,2 };
    ...
    printf("%s:: Helper 3 GPU %d, %d, %d\n", __func__, helper_group_3_devs[0], helper_group_3_devs[1], helper_group_3_devs[2]);
    ...
    for ( int i = 0; i < helper_group_3_num_comm; ++i )
    {
        ...
        CUDACHECK(cudaStreamCreate( &(helper_group_3_streams[i]) ));
    }
    ...
    setenv( "NCCL_GRAPH_FILE", std::getenv("BLINKPLUS_GRAPH_FILE_CHAIN_032") , 1 );
    ...
    setenv( "NCCL_GRAPH_FILE", std::getenv("BLINKPLUS_GRAPH_FILE_CHAIN_012") , 1 );
    ...
    // End timing
    {
      printf("=====End Timing Three Helper======\n");
      auto delta = std::chrono::high_resolution_clock::now() - start;
      double deltaSec = std::chrono::duration_cast<std::chrono::duration<double>>(delta).count();
      deltaSec = deltaSec / num_iters;
      double timeUsec = deltaSec*1.0E6;
