...
int main(int argc, char* argv[])
{
    if ( argc != 6 )
    {
        printf("Usage ./time_nccl_allreduce_user03_3group.out 0 3 NUM_WARMUP NUM_ITER TOTAL_DATA_SIZE\n");
        exit(1);
    }
    ...
    for ( int comm_i = 0; comm_i < helper_group_1_num_comm; ++comm_i )
    {
      ...
      if ( helper_group_1_devs[ comm_i ] == 0 || helper_group_1_devs[ comm_i ] == 3 )
      {
    ...
    for (int iter = 0; iter < num_warmup; iter++) 
    {
        ...
        for ( int i = 0; i < user_group_num_comm; ++i )
        {
            NCCLCHECK(ncclAllReduce((const void*)user_group_sendbuff[ i ], \
                                    ...
                                    chunk_data_size * 2, ncclInt8, ncclSum, \
                                    user_group_comms[i], \
    ...
    // End timing
    {
      ...
      double bw = total_data_size / 3 * 2 * sizeof(uint8_t) / 1.0E9 / deltaSec;
      printf("%s:: Average of %d Iters, data: %d MB,  Elapsed Time: %7.5f (us), BandWidth: %7.5f (GB/s)\n", \
                  __func__, num_iters, int(atoi(argv[5]) / 3 * 2 * sizeof(uint8_t)), timeUsec,  bw);  
    }
